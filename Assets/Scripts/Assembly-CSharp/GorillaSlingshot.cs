...
public class GorillaSlingshot : MonoBehaviour
{
    ...
    void Start()
    {
        grabInteractable = grabPoint.GetComponent<UnityEngine.XR.Interaction.Toolkit.Interactables.XRGrabInteractable>();
        grabInteractable.onActivate.AddListener(OnGrab);
        grabInteractable.onDeactivate.AddListener(OnRelease);
    }

    void OnGrab(UnityEngine.XR.Interaction.Toolkit.Interactors.XRBaseInteractor interactor)
    {
        isGrabbing = true;
        grabOffset = grabPoint.transform.position - interactor.transform.position;
    }

    void OnRelease(UnityEngine.XR.Interaction.Toolkit.Interactors.XRBaseInteractor interactor)
    {
        isGrabbing = false;
